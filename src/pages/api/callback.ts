...
export const POST: APIRoute = async ({ request }) => {
  ...
  const { name, phone, hp } = parsed.data;
  if (hp) return json({ ok: false }, 200); // honeypot caught

  const captchaToken = (body as Record<string, unknown>)['captchaToken'] as string | undefined;
  if (!(await verifyCaptcha(captchaToken))) {
    return json({ ok: false, error: 'Captcha failed' }, 400);
  }

  const fields = { Имя: name, Телефон: phone };
  const subject = 'Заявка на звонок — VS FORNI';
  ...
  logFormSubmission('callback', { name, phone });
